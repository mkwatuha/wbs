...
function formatSMSMessage($messageType,$connection_number,$amount,$billdate){
	  if($messageType){
...
function createSmsTag($phoneNumber,$smsMessage){
    ...
    return '<sms><user>Kimilili</user><password>t123456</password><message>'.$smsMessage.'</message><mobiles>'.$phoneNumber.'</mobiles><senderid></senderid><cdmasenderid></cdmasenderid><group>-1</group><clientsmsid>0</clientsmsid><accountusagetypeid>1</accountusagetypeid></sms>';
...
if(($startcommtype[0]=='bill')||($startcommtype[0]=='disconnection')){
    ...
    if(!$connectionExists){
    insertWaterBillHandler($phone_number,$connection_number,$amount,$pay_before,$commtype,$messageType);
    }

    }else{
      splitDelimitedWaterBillNumbers($phone_number,$connection_number,$amount,$pay_before,$commtype,$messageType);
    }
}
...
$messageCustmomized=formatGeneralMessage($recepient,$additionalDetails,$message);
    ...
    $date_created=date('Y-m-d');
	          ...
	          insertOtherProcessingInfo($recepient,$phone_number,$messageCustmomized,$scheduleId);
              }else{
                  splitDelimitedNumbers($recepient,$phone_number,$messageCustmomized,$scheduleId);
              }
			//
			}
...
function splitDelimitedWaterBillNumbers($phone_number,$connection_number,$amount,$pay_before,$sys_track,$messageType){
       ...
       if(sizeof($actualfieldsArr)<2){
		...
		for($i=0;$i<sizeof($actualfieldsArr);$i++){
            ...
            if((strlen($Number)==10)&&(is_numeric($Number))){
             insertWaterBillHandler($Number,$connection_number,$amount,$pay_before,$sys_track,$messageType);
            }else{
...
function insertWaterBillHandler($phone_number,$connection_number,$amount,$pay_before,$sys_track,$messageType){
        $created_by=$_SESSION['my_useridloggened'];
    $date_created=date('Y-m-d');
  $uuid=gen_uuid();
  $voided=0;
  $message='';
  if($messageType) $message=formatSMSMessage($messageType,$connection_number,$amount,$pay_before);
    $insertSQl= "Insert into sms_billhandle (connection_number,amount,phone_number,pay_before,smsmsgcust_id,proposed_message,date_created,created_by,voided,sys_track,uuid) values
                        ('$connection_number','$amount','$phone_number','$pay_before','$messageType','$message','$date_created','$created_by','$voided','$sys_track','$uuid')";
