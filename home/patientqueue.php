...
,{name:'action'}]
    ...
    proxy: {
        type: 'ajax',
        url : 'buidgrid.php?mprcds=1&fa=$isapproved',
        reader: {
            type: 'json'
        }
    }
  ...
  //////////
    ...
    itemclick: function(dv, record, item, index, e) {
		...
		//
		var diagnosis_type=record.get('diagnosis_type');
		var hospitalnum=record.get('hospital_number');
		var labnum=record.get('queue_name');
		var queuid=record.get('queue_id');
		var personname=record.get('person_fullname');
		var actionitempr=record.get('action');
		var showinpanel='estsearchform';
		        if(actionitempr=='Queue'){
				medicallab_queueFormRevised(person_id,empname,'estsearchform');
				}
				if(diagnosis_type=='Cytology'){
				
				medicallab_cytologyrefFormRevised(hospitalnum,queuid,labnum,personname,showinpanel);
				
				}
				if(diagnosis_type=='Histology'){
				medicallab_histologyFormRevised(hospitalnum,queuid,labnum,personname,showinpanel);
				...
				if(diagnosis_type=='Pap Smear'){
				medicallab_papsmearFormRevised(hospitalnum,queuid,labnum,personname,showinpanel);
